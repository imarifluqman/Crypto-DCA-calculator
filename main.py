...
def calculate_additional_position(old_price, old_qty, new_price, target_price):
  try: 
    old_value = old_price * old_qty
    x = (target_price * old_qty - old_value) / (new_price - target_price)
    return round(x, 6) 
...
st.subheader("🔢 Input Current Position")
old_price = st.number_input("Old Entry Price (e.g. 105000)", value=105000.0) 
old_qty = st.number_input("Old Position Quantity in BTC (e.g. 0.0054)", value=0.0054, format="%.6f")

st.subheader("🎯 Target and Market Info")
target_price = st.number_input("Target Entry Price (e.g. 106300)", value=106300.0) 
new_price = st.number_input("Current Market Price (e.g. 106234.7)", value=106234.7)
